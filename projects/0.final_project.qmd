...
![Schematic of microglial phenotypic switching in response to environmental stimuli.](images/phenotypic_swtich.png){fig-align="center"}
...
![Schematic of Sholl analysis workflow for measuring microglia complexity.](images/sholl_wkflow.png)
...
detailed <- read_csv("/Users/clara-hibbs/Desktop/rclass/data/240521_MG/21-32-07/Imaris/21-32-07_100x_CA1_1_Statistics/21-32-07_100x_CA1_c1_Detailed.csv", col_names = TRUE, skip = 3)
...
![](images/original.png){fig-align="center" width="367"}
...
all_files <- list.files(here("data/Sholl_trim"), pattern = "\\.csv$", full.names = TRUE)
...
data <- separate_wider_delim(df, 
                             cols = filename_copy, 
                             names = c("folder1", "folder2", "id", "folder4", "folder5", "Detailed"), 
                             delim = "/")
glimpse(data)
```
...
write.csv(data, "/Users/clara-hibbs/Desktop/rclass/data/Sholl.csv", row.names = FALSE)
```
...
meta <- read.csv("/Users/clara-hibbs/Desktop/rclass/data/meta_sholl.csv", header = TRUE, stringsAsFactors = FALSE, colClasses = "character")
head(meta)
```
